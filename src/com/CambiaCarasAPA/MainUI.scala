...
class UI(val juego: Juego) extends MainFrame {
  ...
  def mueve(x1: Int, y1: Int, x2: Int, y2: Int) = {
    tablaPrincipal = juego.turno(juego.exchange(tablaPrincipal, x1, y1, x2, y2))
  ...
  def newGame() {
    try {
      ...
      if (dificultad.equals(1) || dificultad.equals(2) || dificultad.equals(3)) {
        ...
        Dialog.showMessage(contents.head, "Dificultad inválida", title = "ERROR")
      }
    } catch {
  ...
  def turno() {
    ...
    tablaPrincipal = juego.turno(juego.exchange(tablaPrincipal, movimiento(0), movimiento(1), movimiento(2), movimiento(3)))
    paint(tablaPrincipal)
  }
}

object CambiaCarasGUI {
